...
constexpr uint32_t frameDots =
    snesquik::ppu::Ppu::dotsPerScanline * snesquik::ppu::Ppu::ntscScanlines;
...
public:
  ...
  void mmioWrite(uint16_t address, uint8_t value) override {
    ...
    if ((address >= 0x210d && address <= 0x2112) ||
        (address >= 0x2126 && address <= 0x2131) || address == 0x2100 ||
        address == 0x2105 || address == 0x212c || address == 0x212d) {
      logFile << "[MMIO]"
              << " frame=" << frameNumber << " reg=$" << std::hex << address
              << " value=$" << int(value) << std::dec << '\n';
    }
  }

  void dmaTransfer(uint8_t channel, uint32_t sourceAddress, uint8_t bbad,
                   uint8_t value, uint16_t vramAddress) override {
    if (!logFile.is_open()) {
      return;
    }

    logFile << "[DMA-XFER]"
            << " frame=" << frameNumber << " ch=" << std::dec << int(channel)
            << " src=$" << std::hex << sourceAddress << " bbad=$" << int(bbad)
            << " value=$" << int(value) << " vmadd=$" << vramAddress << std::dec
            << '\n';
  }
  ...
  void dmaStart(uint8_t channel, uint16_t size) override {
    if (!logFile.is_open())
      return;
    logFile << "[DMA]"
            << " frame=" << frameNumber << " ch=" << int(channel)
            << " size=" << size << '\n';
  }
  ...
  void vblank(bool active) override {
    if (!logFile.is_open())
      return;
    logFile << "[VBLANK]"
            << " frame=" << frameNumber << " active=" << active << '\n';
  }
  ...
  void hdmaWrite(uint8_t channel, uint16_t address, uint8_t value) override {
    if (!logFile.is_open())
      return;
    logFile << "[HDMA]"
            << " frame=" << frameNumber << " ch=" << int(channel) << " reg=$"
            << std::hex << address << " value=$" << int(value) << std::dec
            << '\n';
  }
};
...
void loadSramFile(const std::string &path, std::vector<uint8_t> &sram) {
  ...
  if (data.size() != sram.size()) {
    std::cerr << "warning: SRAM file is " << data.size()
              << " bytes but cartridge SRAM is " << sram.size() << " bytes\n";
  }
}
...
void logFrameState(snesquik::ppu::Ppu &ppu, snesquik::bus::SnesBus &bus,
                   const snesquik::cpu_r5a22::CPU &cpu, uint32_t frame) {
  if (!logFile.is_open()) {
    return;
  }
  ...
  logFile << "  CPU PB:PC=$" << std::hex << static_cast<int>(cpuReg.pb) << ":"
          << static_cast<int>(cpuReg.pc) << " A=$" << static_cast<int>(cpuReg.a)
          << " X=$" << static_cast<int>(cpuReg.x) << " Y=$"
          << static_cast<int>(cpuReg.y) << " S=$" << static_cast<int>(cpuReg.s)
          << " D=$" << static_cast<int>(cpuReg.d) << " DB=$"
          << static_cast<int>(cpuReg.db) << " P=$" << static_cast<int>(cpuReg.p)
          << " E=" << (cpuReg.emulation ? 1 : 0) << std::dec << '\n';

  // PPU registers
  logFile << "  INIDISP=$" << std::hex
          << static_cast<int>(ppu.readRegister(0x2100))
          << " INIDISP_WRITES=" << std::dec << ppu.getInidispWriteCount()
          << " LAST_INIDISP_WRITTEN=$" << std::hex
          << static_cast<int>(ppu.getLastInidispWritten()) << " BGMODE=$"
          << static_cast<int>(ppu.readRegister(0x2105)) << " MOSAIC=$"
          << static_cast<int>(ppu.readRegister(0x2106)) << " TM=$"
          << static_cast<int>(ppu.readRegister(0x212c)) << " TS=$"
          << static_cast<int>(ppu.readRegister(0x212d)) << " TMW=$"
          << static_cast<int>(ppu.readRegister(0x212e)) << " TSW=$"
          << static_cast<int>(ppu.readRegister(0x212f)) << " CGSEL=$"
          << static_cast<int>(ppu.readRegister(0x2130)) << " CGADSUB=$"
          << static_cast<int>(ppu.readRegister(0x2131)) << " SETINI=$"
          << static_cast<int>(ppu.readRegister(0x2133)) << std::dec << '\n';

  // BG scroll registers
  logFile << "  BG1HOFS=$" << std::hex
          << static_cast<int>(ppu.readRegister(0x210d)) << " BG1VOFS=$"
          << static_cast<int>(ppu.readRegister(0x210e)) << " BG2HOFS=$"
          << static_cast<int>(ppu.readRegister(0x210f)) << " BG2VOFS=$"
          << static_cast<int>(ppu.readRegister(0x2110)) << " BG3HOFS=$"
          << static_cast<int>(ppu.readRegister(0x2111)) << " BG3VOFS=$"
          << static_cast<int>(ppu.readRegister(0x2112)) << std::dec << '\n';

  // BG tilemap bases
  logFile << "  BG1SC=$" << std::hex
          << static_cast<int>(ppu.readRegister(0x2107)) << " BG2SC=$"
          << static_cast<int>(ppu.readRegister(0x2108)) << " BG3SC=$"
          << static_cast<int>(ppu.readRegister(0x2109)) << " BG4SC=$"
          << static_cast<int>(ppu.readRegister(0x210a)) << " BG12NBA=$"
          << static_cast<int>(ppu.readRegister(0x210b)) << " BG34NBA=$"
          << static_cast<int>(ppu.readRegister(0x210c)) << " VMADD=$"
          << static_cast<int>(ppu.readRegister(0x2116) |
                              (ppu.readRegister(0x2117) << 8))
          << " CGADD=$" << static_cast<int>(ppu.readRegister(0x2121))
          << std::dec << '\n';

  // Window registers
  logFile << "  W12SEL=$" << std::hex
          << static_cast<int>(ppu.readRegister(0x2123)) << " W34SEL=$"
          << static_cast<int>(ppu.readRegister(0x2124)) << " WOBJSEL=$"
          << static_cast<int>(ppu.readRegister(0x2125)) << " WH0=$"
          << static_cast<int>(ppu.readRegister(0x2126)) << " WH1=$"
          << static_cast<int>(ppu.readRegister(0x2127)) << " WH2=$"
          << static_cast<int>(ppu.readRegister(0x2128)) << " WH3=$"
          << static_cast<int>(ppu.readRegister(0x2129)) << " WBGLOG=$"
          << static_cast<int>(ppu.readRegister(0x212a)) << " WOBJLOG=$"
          << static_cast<int>(ppu.readRegister(0x212b)) << std::dec << '\n';

  // NMI/IRQ
  logFile << "  NMI_EN=$" << std::hex << static_cast<int>(bus.readMmio(0x4200))
          << " VTIME=$"
          << static_cast<int>(bus.readMmio(0x4209) |
                              (bus.readMmio(0x420a) << 8))
          << " HTIME=$"
          << static_cast<int>(bus.readMmio(0x4207) |
                              (bus.readMmio(0x4208) << 8))
          << " DMAEN=$" << static_cast<int>(bus.readMmio(0x420b)) << " HDMAEN=$"
          << static_cast<int>(bus.readMmio(0x420c)) << std::dec << '\n';
  ...
  logFile << "  debugFlags=$" << std::hex
          << static_cast<int>(ppu.debugFlagsMask()) << std::dec << '\n';
  ...
  logFile << "  APU_R=[" << std::hex << static_cast<int>(apl.lastRead[0]) << ' '
          << static_cast<int>(apl.lastRead[1]) << ' '
          << static_cast<int>(apl.lastRead[2]) << ' '
          << static_cast<int>(apl.lastRead[3]) << "]"
          << " W=[" << static_cast<int>(apl.lastWrite[0]) << ' '
          << static_cast<int>(apl.lastWrite[1]) << ' '
          << static_cast<int>(apl.lastWrite[2]) << ' '
          << static_cast<int>(apl.lastWrite[3]) << "]"
          << " RDcnt=[" << std::dec << apl.readCount[0] << ' '
          << apl.readCount[1] << ' ' << apl.readCount[2] << ' '
          << apl.readCount[3] << "]"
          << " WRcnt=[" << std::dec << apl.writeCount[0] << ' '
          << apl.writeCount[1] << ' ' << apl.writeCount[2] << ' '
          << apl.writeCount[3] << "]"
...
// Handles debug/system keys (PPU debug toggles, logging, save-state). Returns
// true if the key was consumed here so it should not also be treated as a
// controller button.
bool handleSystemKey(InputContext &input, int key, bool pressed) {
  ...
  case SDLK_F3:
    if (input.ppu)
      input.ppu->setDebugFlags(input.ppu->debugFlagsMask() ^ Ppu::DebugOnlyBG3);
    return true;
  case SDLK_F4:
    if (input.ppu)
      input.ppu->setDebugFlags(input.ppu->debugFlagsMask() ^
...
void handleControllerInput(InputContext &input, int key, bool pressed) {
  ...
  input.bus->setButton(*button, pressed);
}
...
void handleKey(void *userData, int key, bool pressed) {
  ...
  if (handleSystemKey(*input, key, pressed)) {
    return;
  }
  handleControllerInput(*input, key, pressed);
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 2; i < argc; ++i) {
    if (isOption(argv[i], "--probe") && i + 1 < argc) {
      ...
      probeOptions.gsuTraceFrame =
          static_cast<uint32_t>(std::strtoul(argv[++i], nullptr, 10));
      probeOptions.gsuTraceCount =
          static_cast<uint32_t>(std::strtoul(argv[++i], nullptr, 10));
    } else if (isOption(argv[i], "--probe-poke") && i + 3 < argc) {
      const uint32_t frame =
          static_cast<uint32_t>(std::strtoul(argv[++i], nullptr, 10));
      const uint32_t addr =
          static_cast<uint32_t>(std::strtoul(argv[++i], nullptr, 16));
      const uint8_t val =
  ...
  if (probeMode) {
    ...
    std::cout << result.message << ": reset PC=$" << std::hex << result.resetPc
              << std::dec << " cycles=" << result.cpuCycles << '\n';
    return 0;
  }
  ...
  const auto parsed = snesquik::cartridge::parseRomImage(
      std::span<const uint8_t>(file.data(), file.size()));
  if (!parsed) {
  ...
  bus.attachPpu(&ppu);
  bus.attachCartridge(parsed->rom, parsed->header.map,
                      parsed->header.declaredRamSizeBytes());
  ...
  std::cout << parsed->header.title << " ("
            << snesquik::cartridge::cartridgeMapName(parsed->header.map)
            << ")\n";
  if (parsed->header.chipset >= 0x03 && !parsed->header.hasSuperFx() &&
      !parsed->header.hasSa1() && !parsed->header.hasDsp() &&
      !parsed->header.hasSdd1()) {
    std::cerr
        << "warning: cartridge requires an enhancement coprocessor (chipset $"
  ...
  if (!renderer.initialize("snesquik", snesquik::ppu::Ppu::screenWidth,
                           snesquik::ppu::Ppu::screenHeight)) {
    std::cerr << "renderer init failed: " << renderer.lastError() << '\n';
    return 1;
  }
  InputContext input{&bus, &ppu, false, false};
  ...
  const double framePeriod =
      static_cast<double>(perfFreq) * 357368.0 / 21477272.0;
  double frameDeadline =
      static_cast<double>(SDL_GetPerformanceCounter()) + framePeriod;
  ...
  while (running) {
    ...
    while (dotsThisFrame < frameDots) {
      ...
      bus.checkIrqCrossing(prevH, prevV, ppu.horizontalCounter(),
                           ppu.verticalCounter());
      ...
      if (!nmiRequested &&
          ppu.verticalCounter() >= static_cast<uint16_t>(ppu.visibleHeight())) {
        bus.setVblank(true);
    ...
    if (audioDevice != 0) {
      ...
      const double queuedSeconds =
          static_cast<double>(SDL_GetQueuedAudioSize(audioDevice)) /
          audioBytesPerSecond;
      if (!samples.empty() && queuedSeconds >= 0.25) {
        ...
        const double ratioTarget =
            std::clamp(1.0 + rateBias + error * 0.01, 0.97, 1.03);
        ...
        if (std::abs(ratio - 1.0) >= 0.0005) {
          ...
          const int outFrames =
              std::max(1, static_cast<int>(inFrames * ratio + 0.5));
        ...
        SDL_QueueAudio(audioDevice, out,
                       static_cast<Uint32>(outCount * sizeof(int16_t)));
        if (audioTee.is_open()) {
          audioTee.write(
    ...
    if (static_cast<double>(perfNow) > frameDeadline + 4.0 * framePeriod) {
      ...
      while (static_cast<double>(perfNow) < frameDeadline) {
        const double remainingMs =
            (frameDeadline - static_cast<double>(perfNow)) * 1000.0 /
    ...
    if (elapsed >= 1000) {
      currentFps = static_cast<float>(fpsFrameCount) * 1000.0f /
      ...
      if (audioVerbose && (audioUnderruns == 0 && audioOverruns == 0)) {
        std::cerr << "audio: ur=0 ov=0 produced=" << audioProducedLastSecond
                  ...
                  << " ratio=" << audioRatioLastSample << " fps=" << currentFps
                  << '\n';
      }
      if (audioUnderruns > 0 || audioOverruns > 0) {
        std::snprintf(
            title, sizeof(title), "snesquik [F:%.1f] [AUD ur:%u ov:%u] %s%s",
            currentFps, audioUnderruns, audioOverruns,
            input.logActive ? "[LOG] " : "", parsed->header.title.c_str());
        ...
        std::snprintf(title, sizeof(title), "snesquik [F:%.1f] %s%s",
                      currentFps, input.logActive ? "[LOG] " : "",
