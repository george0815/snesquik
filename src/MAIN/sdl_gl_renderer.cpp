...
using GlVertexAttribPointer = void (*)(GLuint, GLint, GLenum, GLboolean,
                                       GLsizei, const void *);
...
GLuint compileShader(GLenum type, const char *source, std::string &error) {
  ...
  GLsizei length = 0;
  gl.getShaderInfoLog(shader, static_cast<GLsizei>(log.size()), &length,
                      log.data());
...
SdlGlRenderer::~SdlGlRenderer() { shutdown(); }

bool SdlGlRenderer::initialize(const char *title, int sourceWidth,
                               int sourceHeight, int scale) {
  shutdown();
  width = sourceWidth;
  height = sourceHeight;
  ...
  window = SDL_CreateWindow(
      ...
      height * scale, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
  if (!window) {
    setError(SDL_GetError());
    shutdown();
    return false;
...
bool SdlGlRenderer::pollEvents() {
  ...
  while (SDL_PollEvent(&event) != 0) {
    ...
    if ((event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) && keyCallback &&
        event.key.repeat == 0) {
      keyCallback(keyCallbackUserData, event.key.keysym.sym,
    ...
    if ((event.type == SDL_CONTROLLERBUTTONDOWN ||
      ...
      if (controller) {
        ...
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_X:
          keyCallback(keyCallbackUserData, SDLK_s,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_Y:
          keyCallback(keyCallbackUserData, SDLK_a,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_A:
          keyCallback(keyCallbackUserData, SDLK_x,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_B:
          keyCallback(keyCallbackUserData, SDLK_z,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_LEFTSHOULDER:
          keyCallback(keyCallbackUserData, SDLK_q,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_RIGHTSHOULDER:
          keyCallback(keyCallbackUserData, SDLK_w,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_BACK:
          keyCallback(keyCallbackUserData, SDLK_BACKSPACE,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_START:
          keyCallback(keyCallbackUserData, SDLK_RETURN,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;

        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_UP:
          keyCallback(keyCallbackUserData, SDLK_UP,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;

        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_DOWN:
          keyCallback(keyCallbackUserData, SDLK_DOWN,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;

        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_LEFT:
          keyCallback(keyCallbackUserData, SDLK_LEFT,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;

        case SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_DPAD_RIGHT:
          keyCallback(keyCallbackUserData, SDLK_RIGHT,
                      event.type == SDL_CONTROLLERBUTTONDOWN);
          break;
        }
      }
...
void main() {
  ...
  const GLuint fs =
      compileShader(GL_FRAGMENT_SHADER, fragmentShader, shaderError);
  if (!fs) {
    gl.deleteShader(vs);
  ...
  if (ok != GL_TRUE) {
    std::array<char, 1024> log{};
    GLsizei length = 0;
    gl.getProgramInfoLog(program, static_cast<GLsizei>(log.size()), &length,
                         log.data());
    setError("shader link failed: " +
             std::string(log.data(), static_cast<size_t>(length)));
    return false;
  }
...
void SdlGlRenderer::setError(const std::string &message) { error = message; }
